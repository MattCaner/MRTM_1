...
def output_and_save_only_prediction(model: ElasticTransformer, input: Tensor, output: Tensor, device: int, visualize_index: int = 0, location = ".", name = "fig"):
    decoder_input = torch.cat((torch.unsqueeze(input[0],dim=0),output[0:-1]))
...
def long_range_output_and_save(model: ElasticTransformer, input: Tensor, output: Tensor, device: int, visualize_index: int = 0, location = ".", name = "fig"):
    decoder_input = torch.cat((torch.unsqueeze(input[0],dim=0),output[0:-1]))
