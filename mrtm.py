...
class MRTM(nn.Module):
    ...
    def run(self, total_epochs, increment, device_id, train_dataset, vertical_control_delay, flow_control_dataset,
        ...
        while epochs < total_epochs:
            ...
            if epochs > 0 and epochs % backup_delay == 0 and backup == True:
                ...
                file = open(self.dump_folder+'/'+self.dump_name+'_'+date_format+'.dump', 'wb')
    ...
    def generate_report(self, dataset, device_id, path = "./", name = ""):
        ...
        if directory_created:
            text = ""
            for i in range(8):
                text_to_append = et.output_and_save_only_prediction(self.t,dataset[0][0],dataset[0][2],device_id,i, location = path, name = name)
                text_to_append += et.long_range_output_and_save(self.t,dataset[0][0],dataset[0][2],device_id,i, location = path, name = name)
            ...
            report_txt = open(path+'/'+name+'_report.txt',"w")
            report_txt.write(text)

            report_txt.write('\nStructure\n')
            report_txt.write('Encoder stack: \n')
            for e in self.t.encoder_stack.encoders:
                report_txt.write(str(len(e.mha.heads))+"\n")

            print('Decoder stack: \n')
            for d in self.t.decoder_stack.decoders:
                report_txt.write(str(len(d.self_mha.heads)) + ' ' + str(len(d.ed_mha.heads)) + '\n')
